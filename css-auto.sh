...
# Función para mostrar un ejemplo y esperar la confirmación del usuario
mostrar_ejemplo() {
  local inicio=$1
  local fin=$2
  cat -n "$archivo" | sed -n "${inicio},${fin}p"
...
while true; do
  ...
  case $opcion in
    ...
    2)
      ...
      ;;
    3)
      # Mostrar un ejemplo de líneas específicas
      read -p "Mostrar ejemplo desde la línea: " inicio_ejemplo
      read -p "Mostrar ejemplo hasta la línea: " fin_ejemplo
      mostrar_ejemplo "$inicio_ejemplo" "$fin_ejemplo"
      ;;
    4)
      # Salir del programa
      exit 0
